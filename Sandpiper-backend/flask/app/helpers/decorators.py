...
def token_required(func):
    """
    A decorator that validates the JWT token in the Authorization header.
    Sets request.user_id to the user ID from the token.
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            # Set user ID in request object - check for both person_id and person_entity_id
            user_id = parsed_token.get('person_entity_id') or parsed_token.get('person_id')
            if not user_id:
                return get_failure_response(message='Token missing person ID', status_code=401)
            ...
            logger.exception(e)
            return get_failure_response(message='Invalid token', status_code=401)
